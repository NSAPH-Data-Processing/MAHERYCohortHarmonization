...
#' 
#' @export
#' @importFrom MAHERYCohortHarmonization authenticate_google_drive
#' @importFrom googledrive drive_get, as_id
#' @importFrom lubridate ymd
has_drive_file_changed <- function(file_id, last_known_run_time=ymd("2025-04-01 00:00:00", tz = "UTC")) {
...
has_drive_file_changed(test_file, ymd_hms("2025-04-01 00:00:00", tz = "UTC"))
```
...
test_that("create_datapaths works", {
  ...
  withr::local_envvar(list(R_PROFILE_USER = tempfile()))  # Prevent project detection
  here::i_am("dummy.Rproj") # Simulate project root marker

  # Source the function from your R/ folder or assume it's already available
  create_datapaths()
...
With this in mind, we can likely dedicate one section to each of the files,
...
datxl <- read_excel(dat$local_path)
skimr::skim(datxl)
```
